...
class WorldViewer(ShowBase):
    def __init__(self, dem_data=None, pixel_size=None):
        ShowBase.__init__(self)
        
        # Store DEM data and pixel size
    ...
    def create_terrain_mesh(self):
        if self.dem_data is None:
            # Fall back to procedural terrain if no DEM provided
            return self.create_procedural_terrain()
    ...
    def create_procedural_terrain(self):
        # Copy the original create_terrain_mesh code here
        # and rename it to create_procedural_terrain
        format = GeomVertexFormat.getV3n3()
        vdata = GeomVertexData('terrain', format, Geom.UHStatic)
        vertex = GeomVertexWriter(vdata, 'vertex')
        normal = GeomVertexWriter(vdata, 'normal')
...
"""
    ...
    def height_at(self, x, y):
        ...
        array_x = int((x + half_width) / self.pixel_size)
        array_y = int((y + half_height) / self.pixel_size)
        
        # Clamp to valid indices
        array_x = max(0, min(array_x, self.dem_data.shape[1] - 1))
        array_y = max(0, min(array_y, self.dem_data.shape[0] - 1))
        
        return float(self.dem_data[array_y, array_x])
...
def main():
    ...
    try:
        with rasterio.open(dem_file) as dem_dataset:
            ...
            # Calculate pixel size based on actual terrain dimensions
            total_length = 17000  # 17km in meters
            pixel_size = total_length / dem_data.shape[0]
            
            app = WorldViewer(dem_data, pixel_size)
            app.run()
    except Exception as e:
        print(f"Failed to load DEM, falling back to procedural terrain: {e}")
        app = WorldViewer()
        app.run()
